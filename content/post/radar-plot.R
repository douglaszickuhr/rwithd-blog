radar_plot <- function(plot.data,
  ...
  if (label.gridline.min==TRUE) {
    base <- base + geom_text(aes(x=x,y=y,label=text),data=gridline$min$label,size=grid.label.size, hjust=1) }
  
  if (label.gridline.mid==TRUE) {
  base <- base + geom_text(aes(x=x,y=y,label=text),data=gridline$mid$label,size=grid.label.size, hjust=1) }
  
  if (label.gridline.max==TRUE) {
  base <- base + geom_text(aes(x=x,y=y,label=text),data=gridline$max$label,size=grid.label.size, hjust=1) }
